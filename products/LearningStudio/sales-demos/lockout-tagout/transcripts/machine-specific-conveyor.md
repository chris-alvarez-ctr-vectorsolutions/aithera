# Machine-Specific Lockout/Tagout Procedure — Conveyor

<!--
  SOURCE TRANSCRIPT — verbatim. Do not edit the scene text.

  This is the authored source for the learning object revealed by the
  AI-generation demo. Scene text is reproduced exactly as supplied; the
  course.md entry is generated FROM this file so the two can't drift.

  Format:
    - One `## scene <n>` heading per scene, numbered from 1.
    - The paragraph under it is that scene's transcript, verbatim.
    - `image:` and `duration:` carry the scene's media and timing.

  To seed a course from this file:
    node _kit/transcript-to-md.js lockout-tagout/transcripts/machine-specific-conveyor.md

  See _kit/README.md → "Seeding an LO from a transcript".
-->

meta:
  title: Machine-Specific Lockout/Tagout Procedure — Conveyor
  objective: Apply your facility's machine-specific conveyor procedure to identify every energy source, isolate and verify zero energy, and restore the equipment safely
  image-pattern: NewLO-scene{n}-img.png
