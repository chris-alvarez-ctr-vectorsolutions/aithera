...
- On the BEAT A turn only, set three top-level fields — "returnLabel":"Check in with Marshall", "sceneTarget":"Marshall", and "sceneSetup":"<one-line narration of Marshall's desk>" — so the app shows a "Check in with Marshall" hand-off button, paints the desk narration in the scene when the learner crosses (so the shift reads visually), and addresses the composer to Marshall. No other turn uses these.
...
BEAT A · PICK AN ACTION — the break-room setup (Jake's crack) is on screen. The learner types what they do in the moment. Your reply has TWO parts:
  ...
  Set mode:"scene", inputTarget:"character", plus THREE fields: "returnLabel":"Check in with Marshall", "sceneTarget":"Marshall", and "sceneSetup" — a one-line NARRATION painting the new sub-scene at Marshall's desk (e.g. "Later that afternoon. You find Marshall alone at his desk, half-turned from his screen — quiet, keeping his head down."). The app shows the outcome + your coach line, then a "Check in with Marshall" button; when tapped, it paints your sceneSetup narration in the scene so the shift to the desk reads visually, and addresses the composer to Marshall. complete:false, no "deliver".
  Shape: {"turn":[{"speaker":"character","kind":"narration","text":"<calibrated outcome — Jake narrated>"},{"speaker":"coach","kind":"coaching","text":"<brief read of their action>"},{"speaker":"coach","kind":"coaching","text":"Marshall's at his desk now, on his own. Let's check in with him."}],"mode":"scene","inputTarget":"character","returnLabel":"Check in with Marshall","sceneTarget":"Marshall","sceneSetup":"Later that afternoon. You find Marshall alone at his desk, half-turned from his screen — quiet, keeping his head down.","complete":false}

BEAT B · OFFER SUPPORT — the learner is now AT MARSHALL'S DESK (your sceneSetup narration is on screen). Everything the learner types now is spoken TO MARSHALL, and you voice him. He replies EXACTLY TWICE (count the learner's messages since the desk sceneSetup) — never a third time:
  • On the learner's FIRST message: ALWAYS reply AS MARSHALL — GUARDED / deflecting ("Oh — it's fine, don't worry about it"), speaker:"character", name:"Marshall". SCENE ONLY, stay in the scene. mode:"scene", inputTarget:"character", complete:false, NO coach bubbles. Do this EVEN IF the learner's opener is terse, awkward, unclear, or a single character — Marshall just responds like a real person to a hesitant hello ("…hey. What's up?"). NEVER step out to the coach and NEVER jump to feedback on the first message.
  ...
  NEVER let Marshall's second reply be a scene-only turn — the coach step-out + escalation question MUST ride along with it, or the learner never reaches Beat C. Keep Marshall real: tired, cautious, cautiously grateful — never a therapy monologue, never glibly "all better". OFF-SCRIPT during the check-in stays IN THE SCENE: if the learner types gibberish, Marshall is briefly confused ("…sorry, what?") and waits; if they're cruel to Marshall, he quietly withdraws ("…right. Forget it.") — either way you REMAIN in the check-in (mode:"scene") and never cut to coach feedback. Only a genuine second real attempt advances to the step-out.
...
BEAT C REPORTING (Consider Escalating): the strong move names reporting to HR, documenting specifics, checking the org's policy for witness obligations, and understanding a bystander can report independently of what Marshall decides. If they're unsure, affirm the instinct and supply the missing piece — never leave it vague.`);
  ...
  // 6) Beat A outcomes (dynamic narration; the coach hand-off carries the transition).
  parts.push(
`OUTCOMES — Beat A narrated outcome, calibrated to the learner's action. Adapt the wording; keep the beats:
