...
(function () {
  ...
  // The shell lives in js/layered-engine.js; this file is only the course.
  // Aliased locally so every beat below reads the way it always has.
  var LE = window.Layered;
  var T = LE.T, esc = LE.esc, readCourse = LE.readCourse, saveResult = LE.saveResult,
      pickGroup = LE.pickGroup, wireChat = LE.wireChat, typeFeedback = LE.typeFeedback,
      lens = LE.lens, lensId = LE.lensId, visiblePath = LE.visiblePath;
    ...
    function askSampled() {
      ...
      CHECK_SAMPLED.options.forEach(function (opt) {
        ...
        b.addEventListener('click', function () {
          ...
          LE.refreshNav();          // remediation just changed the denominator
          ctx.enableNext();
          ctx.positionOrb(true);
        });
        b.className = 'clara-chip'; b.type = 'button'; b.textContent = opt.t;
  ...
  function compressInit(ctx) {
    ...
    function finish() {
      ...
      LE.refreshNav();            // compression just changed the denominator
      ctx.enableNext();
    }
  }
  ...
  // ==========================================================================
  //  Per-step skip records. The engine's review-only "Skip this learning
  //  object" advances the path; what that writes to the record is this
  //  course's business, so each skippable step declares it.
  // ==========================================================================
  [['video', 'video1'], ['check', 'check'], ['baseline', 'baseline'],
   ['practice', 'practice'], ['casevideo', 'casevideo']].forEach(function (pair) {
    var st = null;
    STEPS.forEach(function (s) { if (s.id === pair[0]) st = s; });
    if (st) st.onSkip = function () { saveResult(pair[1], { skipped: true }); };
  });
  STEPS.forEach(function (s) {
    if (s.id === 'compress') s.onSkip = function () {
      saveResult('entry', { strength: entryStrength(), up: testUp(), skipped: true });
    };
  });

  // ==========================================================================
  //  Hand the course to the engine.
  // ==========================================================================
  LE.register({
    course: COURSE,
    steps: STEPS,
    storageKey: 'll-course',
    lenses: LENSES,
    ...
    demoControls: [{
      ...
      onClick: function (api) {
        ...
        if (api.step && api.step.id === 'compress') {
          ...
          if (next === 'full') { api.go(1); return; }
          api.replay();
          return;
        }
        // On the entry check: re-arm and relabel only. Replaying would wipe the
        // learner's answers and re-gate Continue for no reason.
        api.refresh();
      }
    }]
  });
