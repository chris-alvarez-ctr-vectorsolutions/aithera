...
**2a. What this content actually is, and why it stays flat text for now.** Clarified by product after the fact: what lands here is a **block of prose describing whatever immediately preceded the scenario** — a video the learner watched, a reading, a section of a course. Not a structured record of a learning object. So the prompt block is deliberately modality-neutral ("what the learner just saw or did", never "completed module"), and **any single field is a complete authoring** — an author can describe a video in *What it covered* and leave the rest empty, which is the shape in use today. A more elegant representation, and the production path that pulls this in automatically rather than having it hand-entered, are both explicitly future work. **Do not restructure the flat text into a richer schema on the assumption that it was a shortcut — it is the agreed interim shape.**
