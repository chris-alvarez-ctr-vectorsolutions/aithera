...
- **Opt-outs** (set `window.TOOLBOX` before the include): `{ comments: false }` = flow map only, which is what dev-handoff builds use; `{ flowMap: false }` = comment widget only, which is how the `Convergence Small UI Improvements` sibling pages are set up since `index.html` owns the flow map. `?toolbox=off` suppresses both for a clean screenshot. At review time the dock's **×** dismisses the whole review cluster (pill, minimized handle, comment pins/panels, flow map) for that page view; a reload brings it back.
