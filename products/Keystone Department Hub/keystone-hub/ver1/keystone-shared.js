...
(function () {
  ...
  // Plain product name — the task table's Product column and anywhere a
  // source needs to be spelled out. Two-letter abbreviations tested badly in
  // UX research (nobody read "TS"/"GT" as a product), so task surfaces always
  // name the product in full.
  function srcName(sourceId) {
    var s = window.KEYSTONE.SOURCES[sourceId];
    return s ? s.name : '';
  }

  // opts.full spells the product out instead of abbreviating it. Compact
  // multi-source displays (a person's entitlements, a widget's source list)
  // still use the short form — that's where the abbreviation earns its space.
  function srcChip(sourceId, opts) {
    var s = window.KEYSTONE.SOURCES[sourceId];
    if (!s) return '';
    opts = opts || {};
    return '<span class="kx-src-chip' + (opts.full ? ' kx-src-chip--full' : '') +
      '" title="' + attr(s.name) + '" style="background:' + s.bg + ';color:' + s.color + '">' +
      '<span class="dot"></span>' + esc(opts.full ? s.name : s.short) + '</span>';
  }

  // Rounded-square app tile with mono initials, in the left rail's "Pinned
  // apps" style. NOTE: currently unused — the task table's App column used to
  // render these, and now names the product in full instead (see srcName).
  ...
  // Priority = the NAMED BAND, never the raw number. The 0–100 score still
  // exists on the task and still drives ranking and sort order; it is simply
  // never shown. The band scheme comes from the Prioritization Settings page
  // (see prioBand below), so renaming or retuning a band there relabels and
  // recolours every task row here.
  function prioBadge(task) {
    var band = prioBand(task.priorityScore);
    return '<span class="kx-prio" style="background:' + band.bg +
      ';color:' + band.fg + '">' + esc(band.label) + '</span>';
  }
  ...
  // v3 = the four named default buckets. The bump is deliberate: a saved v2
  // config would pin the old three-band default and hide the new one from
  // anyone who had already opened the settings page.
  var PRIO_KEY = 'keystone.priorityConfig.v3';
  ...
  // Four bands ramp red → orange → amber → green. (It used to end on blue,
  // which broke the heat ramp by putting a cool tone at the calm end.)
  var BAND_PALETTES = {
    2: [TONE_RED, TONE_GREEN],
    3: [TONE_RED, TONE_YELLOW, TONE_GREEN],
    4: [TONE_RED, TONE_ORANGE, TONE_YELLOW, TONE_GREEN],
  ...
  // The shipped default recommendation, so a customer is never confronted with
  // raw numbers out of the box. Critical sits at 85 so it reads as a genuine
  // exception tier rather than a synonym for High.
  var DEFAULT_BAND_LIST = [
    { id: 'b-crit', name: 'Critical', min: 85 },
    { id: 'b-high', name: 'High',     min: 70 },
    { id: 'b-med',  name: 'Medium',   min: 40 },
  ...
  /* =====================================================================
     ...
     One definition, two surfaces: the Agency Intelligence canvas where an author
     lays a dashboard out (agency-intel-canvas.js) and the Hub where that
     dashboard is published (hub-hero.js). Both pages load this file, which is
     the point — a published dashboard has to come back at the size it was built
     at, and that only holds if both ends measure in the same units.

     Cards tile 12 columns with a fixed row track. Width snaps in whole columns,
     height in whole rows, so card borders line up across a row. Every card
     carries an EXPLICIT row span: letting an un-resized card fall back to `auto`
     stretches its whole track to fit the tallest content in it, which undoes the
     alignment for every card beside it. A widget that has never been resized
     starts at the per-kind default below.

     ROW and GAP are duplicated in CSS (.cp-grid, .kx-pubgrid). Change them here
     and there together — the resize drag snaps off these numbers.
     ===================================================================== */
  var GRID = {
    COLS: 12,
    MIN_W: 3, MAX_W: 12,
    ROW: 48,        // height of one row track, px
    GAP: 14,        // row-gap / column-gap on the canvas grid
    MIN_H: 3, MAX_H: 10
  };

  // Starting height per widget kind, in rows. A KPI is one big number; a table
  // needs room for a header, a handful of rows and its pager.
  var DEFAULT_H = {
    kpi: 3, summary: 4, text: 3, progress: 4,
    bar: 5, hbar: 5, stack: 5, line: 5, donut: 5, pair: 5,
    scatter: 6, metrics_table: 6, table: 7
  };

  function clampW(n) { return Math.max(GRID.MIN_W, Math.min(GRID.MAX_W, Math.round(n || 0))); }
  function clampH(n) { return Math.max(GRID.MIN_H, Math.min(GRID.MAX_H, Math.round(n || 0))); }

  function widgetW(w) { return clampW((w && w.w) || 6); }

  // `kind || viz` is the union of both vocabularies: the Hub renders by `kind`,
  // the canvas by `viz`, and text widgets carry both.
  function widgetH(w) {
    if (!w) return 5;
    return clampH(w.h || DEFAULT_H[w.kind || w.viz] || 5);
  }

  // Pixel height of an H-row span, counting the gaps between its tracks.
  function rowsToPx(h) { return h * GRID.ROW + (h - 1) * GRID.GAP; }

  // Width of one column, measured off a live grid so a resize drag tracks the
  // cursor at any canvas width.
  function colWidth(gridEl) {
    var total = gridEl ? gridEl.getBoundingClientRect().width : 0;
    return (total - (GRID.COLS - 1) * GRID.GAP) / GRID.COLS;
  }

  // The spoken form of a card's size — shared by the aria-label and the live
  // region so a screen reader hears the same words either way.
  function sizeLabel(gw, gh) {
    return gw + ' of 12 columns wide, ' + gh + ' row' + (gh === 1 ? '' : 's') + ' tall';
  }
  ...
  window.KX = {
    ...
    clampW: clampW, clampH: clampH,
    widgetW: widgetW, widgetH: widgetH,
    rowsToPx: rowsToPx, colWidth: colWidth, sizeLabel: sizeLabel
  };
})();
