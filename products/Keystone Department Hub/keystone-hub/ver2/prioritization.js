...
(function () {
  ...
  function formulaHtml() {
    var w = cfg.weights;
    return '<b>ranking</b> = <em>' + w.time + '</em> × time_weight + <em>' + w.importance +
      '</em> × type_importance + <em>' + w.effort + '</em> × effort' +
      '<div class="footnote">This blend ranks tasks behind the scenes and decides which priority band ' +
      'each one lands in. People working the task list see the band name only — never the underlying ' +
      'number.</div>';
  }
    ...
    if (previewTab === 'types') {
      ...
      body = rows.map(function (r) {
        ...
        return '<div class="pf-preview-task pf-preview-type">' +
          ...
          // The band NAME is the readout — the underlying score still drives the
      ...
      body = scored.map(function (row) {
        ...
        return '<div class="pf-preview-task">' +
          ...
          '<div class="pf-preview-score" style="background:' + tone.bg + ';color:' + tone.fg + '">' +
          esc(tone.label) + '</div>' +
          '<div class="pf-preview-meta">' + esc(meta) + '</div></div>';
      }).join('') +
