...
(function () {
  ...
  var state = {
    ...
    /* ---- canvas layout editing (see the LAYOUT EDITING section below) ----
       `grab` is the keyboard pick-up: { id, w, h, index } snapshotted at pick-up
       so Escape can put the card back. Arrow keys apply changes immediately —
       you have to SEE the move — and Escape rewinds to this snapshot.
       `focusWidgetId` survives the re-render that every commit triggers, so the
       card you were moving still has focus afterwards.
       `liveMsg` is what the aria-live region announces on the next render. */
    grab: null,
    focusWidgetId: null,
    liveMsg: '',
    editingName: false,
  ...
  function dashboardsTab() {
    ...
    // The paginator states the range itself ("1 - 10 of 69"), so there is no
    // separate "Showing x-y of z" caption to keep in sync with it.
    var pager = list.length && totalPages > 1
  ...
  function buildHtml() {
    ...
    if (state.mode === 'report') canvas = reportPreviewHtml(d);
      ...
      canvas =
        // Instructions the cards point at with aria-describedby, plus the region
        // that speaks every reorder/resize. Both are screen-reader only: the
        // pointer affordances explain themselves, the keyboard ones cannot.
        (!locked
          ? '<p id="cpGridHelp" class="cp-sr-only">Press Enter to pick this widget up. ' +
            'Then use the arrow keys to move it in the order, Shift with the arrow keys to resize it, ' +
            'Enter to drop it, or Escape to cancel.</p>' +
            '<div id="cpGridLive" class="cp-sr-only" role="status" aria-live="polite" aria-atomic="true"></div>'
          : '') +
        '<div class="cp-grid"' + (!locked ? ' id="cpGrid"' : '') + '>' + widgets.map(function (w, i) {
          return KXCanvas.widgetCard(w, {
            editable: !locked,
            selected: state.selectedId === w.id,
            grabbed: !!(state.grab && state.grab.id === w.id),
            index: i,
  ...
  function hydrate() {
    /* Layout editing re-renders on every keystroke, which drops focus to <body>
       and rebuilds the live region. Put both back before anything else, so a
       keyboard reorder feels like one continuous interaction. */
    if (state.focusWidgetId) {
      var want = state.focusWidgetId;
      state.focusWidgetId = null;
      document.querySelectorAll('.cpw-card[data-widget-id]').forEach(function (c) {
        if (c.getAttribute('data-widget-id') === want) c.focus({ preventScroll: true });
      });
    }
    if (state.liveMsg) {
      var liveEl = document.getElementById('cpGridLive');
      if (liveEl) liveEl.textContent = state.liveMsg;
      state.liveMsg = '';
    }
  ...
  /* =====================================================================
     LAYOUT EDITING — drag to resize, drag to reorder, keyboard equivalents

     Cards tile a 12-column grid with a fixed row unit (KXCanvas.GRID). Width
     snaps in whole columns, height in whole rows, so borders always line up.

     The one rule everything here is built around: NEVER call render() mid-drag.
     render() replaces #root.innerHTML wholesale, which would destroy the element
     under the user's finger. So a pointer drag mutates the live DOM directly and
     commits exactly once, on pointerup, through setWidgets().

     The keyboard path is the reverse — it has no element to hold onto, so it
     commits (and re-renders) on every keystroke, then puts focus back on the
     card via state.focusWidgetId and speaks the result through #cpGridLive.
     ===================================================================== */

  var GRID = null;                 // KXCanvas.GRID, resolved lazily at first use
  function grid() { return GRID || (GRID = KXCanvas.GRID); }

  function gridEl() { return document.getElementById('cpGrid'); }

  function widgetIndex(id) {
    var ws = (active() || {}).widgets || [];
    for (var i = 0; i < ws.length; i++) if (ws[i].id === id) return i;
    return -1;
  }

  /* Speak something now, without a re-render. Used during pointer drags and for
     dead-ends ("already the widest") where nothing actually changed. */
  function announceNow(msg) {
    var live = document.getElementById('cpGridLive');
    if (live) live.textContent = msg;
  }

  /* Queue an announcement for after the next render — the commit path, since
     setWidgets() rebuilds the live region along with everything else. */
  function announceNext(msg) { state.liveMsg = msg; }

  // "Overdue inspections, 8 of 12 columns wide, 5 rows tall, position 2 of 6."
  function widgetSay(id, w, h) {
    var ws = (active() || {}).widgets || [];
    var i = widgetIndex(id);
    var wd = i >= 0 ? ws[i] : null;
    var title = wd ? CP.widgetTitle(wd) : 'Widget';
    return title + ', ' + KXCanvas.sizeLabel(
      w != null ? w : KXCanvas.widgetW(wd || {}),
      h != null ? h : KXCanvas.widgetH(wd || {})
    ) + ', position ' + (i + 1) + ' of ' + ws.length + '.';
  }

  /* ---- commits ---- */

  function commitSize(id, w, h) {
    setWidgets(function (ws) {
      return ws.map(function (x) {
        return x.id === id ? Object.assign({}, x, { w: w, h: h }) : x;
      });
    });
  }
  ...
  function commitOrder(order) {
    setWidgets(function (ws) {
      var by = {};
      ws.forEach(function (x) { by[x.id] = x; });
      var out = order.map(function (id) { return by[id]; }).filter(Boolean);
      // Anything the DOM didn't account for keeps its place at the end rather
      // than silently vanishing from the dashboard.
      ws.forEach(function (x) { if (out.indexOf(x) === -1) out.push(x); });
      return out;
    });
  }

  // Order as the DOM currently has it — the source of truth after a drag, since
  // the drag reorders elements in place rather than tracking an index.
  function domOrder() {
    var g = gridEl();
    if (!g) return null;
    return Array.prototype.map.call(
      g.querySelectorAll('.cpw-card[data-widget-id]'),
      function (c) { return c.getAttribute('data-widget-id'); }
    );
  }

  /* ---- the chip that follows the cursor ---- */

  var chipEl = null;
  function showChip(text, x, y) {
    if (!chipEl) {
      chipEl = document.createElement('div');
      chipEl.className = 'cp-drag-chip';
      document.body.appendChild(chipEl);
    }
    chipEl.textContent = text;
    chipEl.style.left = (x + 16) + 'px';
    chipEl.style.top = (y + 16) + 'px';
  }
  function hideChip() {
  ...
  /* ---- pointer: resize from the corner ---- */

  function startResize(e, handle) {
    var card = handle.closest('.cpw-card');
    var g = gridEl();
    if (!card || !g) return;
    e.preventDefault();
    ...
    var startW = +card.getAttribute('data-w-cols') || 6;
    var startH = +card.getAttribute('data-w-rows') || 5;
    // Measured off the live grid, so the drag tracks the cursor at any canvas
    // width — the Agency Intelligence dock changes it.
    var colW = KXCanvas.colWidth(g);
    var w = startW, h = startH;

    card.classList.add('is-resizing');
    showChip(startW + ' × ' + startH, e.clientX, e.clientY);

    function move(ev) {
      var dw = Math.round((ev.clientX - startX) / (colW + G.GAP));
      var dh = Math.round((ev.clientY - startY) / (G.ROW + G.GAP));
      var nw = KXCanvas.clampW(startW + dw);
      var nh = KXCanvas.clampH(startH + dh);
      if (nw !== w || nh !== h) {
        w = nw; h = nh;
        card.style.gridColumn = 'span ' + w;
        card.style.gridRow = 'span ' + h;
        card.setAttribute('data-w-cols', w);
        card.setAttribute('data-w-rows', h);
      }
      showChip(w + ' × ' + h, ev.clientX, ev.clientY);
    }
    ...
    function end() {
      ...
      hideChip();
      if (w === startW && h === startH) return;   // nothing moved — no save, no toast
      state.focusWidgetId = id;
      announceNext(widgetSay(id, w, h));
      commitSize(id, w, h);
    }

    /* On window, not on the handle: a drag that leaves the card — or the window —
       must still finish. Pointer capture is deliberately NOT used, see the note
       on the reorder drag below. */
    window.addEventListener('pointermove', move);
    window.addEventListener('pointerup', end);
    window.addEventListener('pointercancel', end);
  }

  /* ---- pointer: drag the header to reorder ---- */

  function startReorder(e, head) {
    var card = head.closest('.cpw-card');
    var g = gridEl();
    if (!card || !g) return;

    var id = card.getAttribute('data-widget-id');
    var startX = e.clientX, startY = e.clientY;
    var startOrder = domOrder();
    var live = false;   // flips true once past the threshold

    // Which card is under the pointer? Hit-tested against rects rather than
    // elementFromPoint, because the dragged card sits under the cursor for most
    // of the gesture and would shadow everything else.
    function cardAt(x, y) {
      var cards = g.querySelectorAll('.cpw-card');
    ...
    function move(ev) {
      if (!live) {
        // A few pixels of slack so a plain click on the header still reads as a
        // click (the kebab and the range control live in here).
      ...
      var over = cardAt(ev.clientX, ev.clientY);
      if (over) {
        var r = over.getBoundingClientRect();
        // Past the midpoint means "after this one". The grid reflows immediately,
        // so what you see mid-drag is exactly what you get on drop.
        if (ev.clientX - r.left > r.width / 2) g.insertBefore(card, over.nextSibling);
        else g.insertBefore(card, over);
      }
      var order = domOrder() || [];
      showChip('Position ' + (order.indexOf(id) + 1) + ' of ' + order.length, ev.clientX, ev.clientY);
    }
    ...
    function end() {
      ...
      if (!live) return;
      var order = domOrder();
      if (!order || !startOrder || order.join() === startOrder.join()) {
        // Dropped where it started. The DOM may still be untouched, but a
        // re-render is the cheapest way to be sure it matches state.
        render();
        return;
      }
      state.focusWidgetId = id;
      // The commit re-renders and rebuilds the live region, so the message has
      // to be queued rather than spoken — and it reads the NEW order, which
      // only exists after commitOrder lands. Hence the index from the DOM.
      announceNext(CP.widgetTitle(widgetById(id) || {}) + ' moved to position ' +
        (order.indexOf(id) + 1) + ' of ' + order.length + '.');
      commitOrder(order);
    }

    /* Listeners go on window and the drag runs WITHOUT setPointerCapture.
       Capturing on the header looks right until the first reorder: moving the
       card with insertBefore takes it out of the document for an instant, which
       implicitly releases the capture, and every remaining event — including the
       pointerup that commits the drag — is delivered somewhere else. The drop
       then silently never happens. */
    window.addEventListener('pointermove', move);
    window.addEventListener('pointerup', end);
    window.addEventListener('pointercancel', end);
  }

  /* ---- keyboard: pick up, move, resize, drop ---- */

  function layoutKeydown(e) {
    if (state.mode !== 'edit') return;
    if (!e.target.closest) return;
    var card = e.target.closest('.cpw-card.is-editable');
    // Only when the CARD itself holds focus. Anything nested — the kebab, a
    // table's search box, the text-widget editor — owns its own keystrokes.
    if (!card || e.target !== card) return;

    var id = card.getAttribute('data-widget-id');
    var ws = (active() || {}).widgets || [];
    var idx = widgetIndex(id);
    if (idx < 0) return;
    var wd = ws[idx];
    var cw = KXCanvas.widgetW(wd), ch = KXCanvas.widgetH(wd);
    var held = !!(state.grab && state.grab.id === id);
    var key = e.key;
    ...
    if (key === 'Enter' || key === ' ' || key === 'Spacebar') {
      ...
      if (held) {
        state.grab = null;
        announceNext('Dropped. ' + widgetSay(id, cw, ch));
      } else {
        /* Snapshot for Escape. Changes apply live from here on — you have to be
           able to see the move — so cancelling means rewinding to this.

           Store the RAW w/h, which may be undefined, not the resolved cw/ch.
           Writing back a resolved default would materialise an explicit height
           on a widget whose author never set one, and `h` is load-bearing: the
           Hub reads it (anySized) to decide whether a dashboard was laid out by
           hand. Pick up a card, press Escape, and the whole published dashboard
           would silently switch to the authored layout. */
        state.grab = { id: id, w: wd.w, h: wd.h, index: idx };
        announceNext('Picked up. ' + widgetSay(id, cw, ch) +
          ' Arrow keys move it, Shift and arrow keys resize it, Escape cancels.');
      }
      render();
      return;
    }

    /* Cancel — put size and position back where they were at pick-up */
    if (key === 'Escape' && held) {
      e.preventDefault();
      var g0 = state.grab;
      state.grab = null;
      state.focusWidgetId = id;
      // Spoken form needs the resolved size even when the stored one is absent.
      announceNext('Cancelled. ' + CP.widgetTitle(wd) + ' returned to position ' +
        (g0.index + 1) + ' of ' + ws.length + ', ' +
        KXCanvas.sizeLabel(KXCanvas.widgetW(g0), KXCanvas.widgetH(g0)) + '.');
      setWidgets(function (list) {
        var out = list.slice();
        var from = -1;
        out.forEach(function (x, i) { if (x.id === id) from = i; });
        if (from < 0) return list;
        var item = Object.assign({}, out[from]);
        // Restore exactly — including back to "never set", which is not the
        // same state as "set to the default".
        if (g0.w == null) delete item.w; else item.w = g0.w;
        if (g0.h == null) delete item.h; else item.h = g0.h;
        out.splice(from, 1);
        out.splice(Math.min(g0.index, out.length), 0, item);
        return out;
      });
      return;
    }

    // Arrows do nothing until the card is picked up, so ordinary arrow-key
    // scrolling still works when you're just tabbing through the canvas.
    if (!held) return;

    var horiz = key === 'ArrowLeft' || key === 'ArrowRight';
    var vert = key === 'ArrowUp' || key === 'ArrowDown';
    ...
    /* Shift + arrows: resize */
    if (e.shiftKey) {
      var nw = horiz ? KXCanvas.clampW(cw + (back ? -1 : 1)) : cw;
      var nh = vert ? KXCanvas.clampH(ch + (back ? -1 : 1)) : ch;
      if (nw === cw && nh === ch) {
        announceNow(horiz
          ? (back ? 'Already the narrowest width.' : 'Already the full width.')
          : (back ? 'Already the shortest height.' : 'Already the tallest height.'));
        return;
      }
      state.focusWidgetId = id;
      announceNext(KXCanvas.sizeLabel(nw, nh) + '.');
      commitSize(id, nw, nh);
      return;
    }

    /* Arrows: reorder. The grid flows left-to-right and wraps, so up/down are
       the same one-step move as left/right — there is no stable "row above" to
       land on when every card is a different width. */
    var to = idx + (back ? -1 : 1);
    if (to < 0 || to >= ws.length) {
      announceNow(back ? 'Already first.' : 'Already last.');
      return;
    }
    state.focusWidgetId = id;
    announceNext(CP.widgetTitle(wd) + ' moved to position ' + (to + 1) + ' of ' + ws.length + '.');
    setWidgets(function (list) {
      var out = list.slice();
      var item = out.splice(idx, 1)[0];
      out.splice(to, 0, item);
      return out;
    });
  }

  function mountLayoutEditing(root) {
    root.addEventListener('pointerdown', function (e) {
      if (state.mode !== 'edit') return;

      var handle = e.target.closest && e.target.closest('[data-w-resize]');
    ...
    root.addEventListener('keydown', layoutKeydown);

    /* Tabbing away from a held card drops it where it stands (its changes are
       already applied) rather than leaving it stuck in the held state with no
       way to release it. Deferred one tick because every commit re-renders,
       which briefly blurs the card before focus is restored in hydrate(). */
    root.addEventListener('focusout', function () {
      if (!state.grab) return;
      var id = state.grab.id;
      setTimeout(function () {
        if (!state.grab || state.grab.id !== id) return;
        var el = document.activeElement;
        var on = el && el.closest && el.closest('.cpw-card[data-widget-id]');
        if (on && on.getAttribute('data-widget-id') === id) return;
        state.grab = null;
        render();
      }, 0);
    });
  ...
  function wire() {
    ...
    // Canvas layout editing (drag-resize, drag-reorder, keyboard equivalents).
    mountLayoutEditing(root);
    ...
    root.addEventListener('click', function (e) {
      ...
      if (mode) {
        state.mode = mode.getAttribute('data-cp-mode');
