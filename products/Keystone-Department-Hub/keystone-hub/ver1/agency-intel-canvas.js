...
(function () {
  ...
  /* ---------- Layout grid ----------
     Defined once in keystone-shared.js (KX.GRID and friends) because the Hub
     renders these same widgets when a dashboard is published, and a published
     dashboard has to come back at the size it was built at. Aliased here so the
     card code below reads the same as it did when the numbers lived in this file. */
  var GRID    = KX.GRID;
  var clampW  = KX.clampW;
  var clampH  = KX.clampH;
  var widgetW = KX.widgetW;
  var widgetH = KX.widgetH;
  var rowsToPx  = KX.rowsToPx;
  var colWidth  = KX.colWidth;
  var sizeLabel = KX.sizeLabel;
  ...
  function widgetCard(widget, o) {
    ...
    // Grid placement. `report` is the print/PDF path, where a card is laid out
    // one-per-page and a fixed row span would clip it — so height stays auto there.
    var gw = widgetW(widget);
    var gh = widgetH(widget);
    var place = 'grid-column:span ' + gw + (o.report ? '' : ';grid-row:span ' + gh);

    // Position is part of the accessible name because reordering by keyboard is
    // otherwise silent — you need to hear where the card landed.
    var pos = (o.index != null && o.total != null)
      ? ', position ' + (o.index + 1) + ' of ' + o.total
      : '';
    var grabbed = !!o.grabbed;

    return '<div class="cpw-card' + (selected ? ' is-selected' : '') +
      (editable ? ' is-editable' : '') + (grabbed ? ' is-grabbed' : '') +
      '" data-widget-id="' + KX.attr(widget.id) + '" ' +
      'data-w-cols="' + gw + '" data-w-rows="' + gh + '" ' +
      'style="' + place + '" tabindex="0"' +
      (editable
        ? ' role="group" aria-roledescription="Dashboard widget"' +
          ' aria-label="' + KX.attr(title + ' — ' + sizeLabel(gw, gh) + pos) + '"' +
          ' aria-grabbed="' + (grabbed ? 'true' : 'false') + '"' +
          ' aria-describedby="cpGridHelp"'
        : '') + '>' +
      '<div class="cpw-card-head"' + (editable ? ' data-w-drag="' + KX.attr(widget.id) + '"' : '') + '>' +
      (editable
        ? '<span class="cpw-grip" aria-hidden="true" title="Drag to reorder">' +
          micon('drag_indicator', { size: 16 }) + '</span>'
        : '') +
      ...
      // the keyboard equivalent is Shift+arrows on the card itself, announced
      // through the live region, which beats focusing a bare handle.
      (editable && !o.report
        ? '<button class="cpw-resize" data-w-resize="' + KX.attr(widget.id) + '" ' +
          'tabindex="-1" aria-hidden="true" title="Drag to resize"></button>'
        : '') +
      '</div>';
  }
  ...
  window.KXCanvas = {
    ...
    // ---- layout grid (drag-resize / drag-reorder live in the page layer) ----
    GRID: GRID,
    widgetW: widgetW,
    widgetH: widgetH,
    clampW: clampW,
