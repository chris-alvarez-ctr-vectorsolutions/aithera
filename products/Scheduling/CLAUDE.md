...
- **Semantic colors** map to the Vector theme's Lumo tokens (`--ink`, `--primary`, `--error`…); the `--ai` palette (`--ai`, `--ai-mid`, `--ai-deep`, `--ai-grad`, `--ai-grad-3`, `--ai-spark`) is the accent for AI-driven surfaces. As of 2026-09-16 `rules-engine-only` retheme the `--ai` palette to **Vector blue** (`--ai: #0271CE`, matching `--primary`) so AI reads as a native part of the product ("invisible AI") rather than a violet "AI module" — aligning with the AI/Smart-Recommendations rule other products carry. Older Scheduling mocks may still use the earlier purple `--ai` values.
