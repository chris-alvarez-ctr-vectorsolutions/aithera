...
A single step in a [`vwc-stepper`](../stepper/CONTEXT.md). Placed in the parent's `step` slot.
...
|---|---|---|---|
| `id` | `string` | â | **Required.** Unique id used by the parent to identify the step. |
| `complete` | `boolean` | `false` | Shows a checkmark icon. |
| `errorMessage` | `string` | â | Shows an error icon and announces error text. |
