...
```html
<vwc-tiling-grid>
  <vwc-tiling-grid-tile>Default 1Ã1</vwc-tiling-grid-tile>
...
- The grid uses `grid-auto-flow: dense`. Tiles may not render in DOM order visually â the grid may place later tiles in earlier gaps. Intentional for dense layouts.
