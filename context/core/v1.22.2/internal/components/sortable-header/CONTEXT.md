...
| `accessibleName` | `string` | â | `aria-label` for the sort button. |
...
| Event | `detail` | Notes |
|---|---|---|
| `sort-direction-change` | `{ direction: 'asc' \| 'desc' \| null }` | Fires (and bubbles) when clicked. Cycles: `null â 'asc' â 'desc' â null`. |
