...
| `overlayBreakpoint` | `number \| undefined` | â | Automatically switch to overlay mode when the drawer's **own width** (px) is at or below this value. |
| `restoreFocusSelector` | `string \| undefined` | â | CSS selector. Element to focus when drawer closes. |
...
| `theme` | `string` | â | Pass-through theme string. |
...
- **Overlay mode makes `content` slot inert.** When `overlay: true` and `open: true`, all elements in the `content` slot become `inert` (non-interactive, not keyboard-focusable). Intentional â the backdrop blocks interaction with page content.
- **`overlayBreakpoint` compares the drawer's own rendered width**, not the viewport. Useful when the drawer is inside a layout that constrains its width (e.g. alongside a sidenav).
- **`resizable: true` only adds a drag handle.** No built-in `minWidth`/`maxWidth` constraints â apply those yourself via CSS if needed.
- **Focus management:** When the drawer closes, focus returns to the element matching `restoreFocusSelector` â but only if focus was inside the drawer at close time. Set `restoreFocusSelector` to the button that opened the drawer for proper accessibility.
- The close button (`closable`) has `autofocus`. It receives focus when the drawer opens.
