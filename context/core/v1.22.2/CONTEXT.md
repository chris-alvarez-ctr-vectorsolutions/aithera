# @vector-web-components/core â AI Component Guide
...
window.vwcI18n.setLocale('es', {
  'vwc-bread-crumb-nav': { overflow_button_label: 'MÃ¡s' }
});
```
...
- **Form inputs**: `vaadin-checkbox`, `vaadin-checkbox-group`, `vaadin-combo-box`, `vaadin-date-picker`, `vaadin-date-time-picker`, `vaadin-multi-select-combo-box`, `vaadin-number-field`, `vaadin-password-field`, `vaadin-radio-group`, `vaadin-select`, `vaadin-text-area`, `vaadin-text-field`, `vaadin-time-picker`
- **Layout & display**: `vaadin-accordion`, `vaadin-accordion-heading`, `vaadin-accordion-panel`, `vaadin-details`, `vaadin-details-summary`, `vaadin-dialog`, `vaadin-tab`, `vaadin-tabs`, `vaadin-tabsheet`, `vaadin-tooltip`
...
`vaadin-checkbox`, `vaadin-checkbox-group`, `vaadin-radio-group`, and `vaadin-upload` do **not** require `theme="outlined"` â that attribute affects only text-input-style components.
