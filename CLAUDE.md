...
4. Create the **`ver1/`** folder and copy **`base-template/version.html`** (the blank Vector canvas) to `ver1/index.html`. **Do all design work here, not in the root loader `index.html`.**
   ...
   - **Dev notes live in `verN/`, next to the design file.** The flow map fetches `DEV-NOTES.md` **relative to the page**, so for a versioned mock it must sit at `verN/DEV-NOTES.md` (not the feature root). Format: one `## <node-id>` section per flow-map node, with `- bullet` notes under it; they render on each step. Maintain it incrementally as you build (a running bullet list of changes per screen) — at dev-handoff it already *is* the dev notes.
