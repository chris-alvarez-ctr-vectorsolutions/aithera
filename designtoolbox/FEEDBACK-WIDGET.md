...
- **Pick-mode banner (admin controls inline)**: when you enter comment mode, the "Click any element to leave feedback" banner also carries the admin controls directly — an **eye / eye-slash icon toggle** (hide comments for non-admins) and a **Log** link (with an arrow-out icon) to the activity log — instead of hiding them behind a gear/settings popover. (Interacting with either promotes you to admin, the same unlock the old gear did.)
